...
app.post('/event/:id/rsvp', function(req, res) {
  checkPermissionsWithCallback(req, res, function(params) {
	...
	eventsController.userRSVP(req, res);
  }, ['User', 'Admin', 'Superuser']);
});
...
app.post('/event/:id/unrsvp', function(req, res) {
  checkPermissionsWithCallback(req, res, function(params) {
	...
	eventsController.userUnRSVP(req, res);
  }, ['User', 'Admin', 'Superuser']);
});
...
// event-specific resume settings
app.post('/event/:id/resume', upload.single('resume'), function(req, res) {

  checkPermissionsWithCallback(req, res, function(params) {
	eventsService.searchEvents({ _id: req.params.id })
	  .then((result) => {
	    if (result.length == 0) {
	      res.status(400).json('no event found with that id!');
	      return;
        }

        req.body.redirect = '/';
        let resumes = result.elements[0].resumes;
	    let removed = _.remove(resumes, (resume) => resume.user === req.session.cas_username);
	    if (req.body.action === 'None') {
	    	if (removed.length > 0) {
	    		req.body.resumes = resumes;
			    eventsController.update(req, res);
		    }
	    } else if (req.body.action === 'Profile') {
			resumes.push({
